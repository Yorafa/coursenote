...
Let $X$ be a _absolute continuous_ random variable with PDF $f_X$. Let $Y=h(X), h:\R\to \R$ is a differentiable and strictly function. Then $Y$ is also absolutely continuous with PDF $f_Y(y)= \frac{f_X(h^{-1}(y))}{|h'(h^{-1}(y))|}$

## Joint

Let $X,Y$ be jointly _absolute continuous_ random variables with PDF $f_{XY}$. Let $Z=h_1(X,Y),W=h_2(X,Y), h_1,h_2:\R^2\to \R$ are differentiable function. Let one-to-one $h = (h_1,h_2):\R^2\to\R^2$. Then the joint density function $f_{Z,W}(z,w) = f_{X,Y}(h^{-1}(z,w))/|\mathcal{J}(h^{-1}(z,w))|$ where $\mathcal{J}(x,y) = \det\begin{bmatrix}\frac{\partial h_1}{\partial x} & \frac{\partial h_2}{\partial x} \\ \frac{\partial h_1}{\partial y} & \frac{\partial h_2}{\partial y}\end{bmatrix}$ is the determinant of the Jacobian matrix 
